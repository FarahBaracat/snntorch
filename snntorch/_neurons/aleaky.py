...
class ALeaky(LIF):
    """
    First-order adaptive leaky integrate-and-fire neuron model.
    ...
    * :math:`β` - Membrane potential decay rate
    ...
    def __init__(
        ...
        self._init_mem()
        # add a custom adaptation time constant
        self.beta_adapt = torch.exp(-torch.tensor(dt) / torch.tensor(tau_adapt))
        self.threshold_weight = threshold_weight  # weight of adaptation to threshold
        self.base_threshold = self.threshold.clone()  # base threshold value
    ...
    def forward(self, input_, mem=None, threshold_adapt=None):
        if not mem == None:
            self.mem = mem
        if not threshold_adapt == None:
        ...
        # decaying adaptation: on spike increase by threshold_weight, otherwise decay
        self.threshold_adapt = (self.beta_adapt * self.threshold_adapt+ (1 - self.beta_adapt) * spk * self.threshold_weight)
    ...
    def detach_hidden(cls):
        ...
        for layer in range(len(cls.instances)):
            if isinstance(cls.instances[layer], ALeaky):
